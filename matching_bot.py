...
PANEL_CHANNEL_ID = int(os.getenv("PANEL_CHANNEL_ID", "0")) # ★新：パネルを置くチャンネルID
...
def create_panel_embed():
    return discord.Embed(
        ...
        description=(
            ...
            f"📝 **作業**（{COUNT_WORK}人）：集中モード！解散時に作業時間が記録されます\n\n"
...
async def on_ready():
    ...
    # ★新機能：募集パネルの自動パトロール＆設置システム
    panel_channel = bot.get_channel(PANEL_CHANNEL_ID)
    if panel_channel:
        print(f"🔎 {panel_channel.name} チャンネルの募集パネルをチェック中...", flush=True)
        has_panel = False
        
        # 過去10件のメッセージを調べて、すでにBotのパネル（ボタン）があるか確認
        async for message in panel_channel.history(limit=10):
            if message.author == bot.user and message.components:
                # ボタンのカスタムIDが一致するかチェック
                if any(component.custom_id == "btn_chat" for action_row in message.components for component in action_row.children):
                    has_panel = True
                    print("✅ 既に有効な募集パネルが存在します。自動設置をスキップします。", flush=True)
                    break
        
        # パネルが見つからなければ、勝手に新規投稿する
        if not has_panel:
            print("🚨 募集パネルが見つかりません。自動設置を開始します...", flush=True)
            embed = create_panel_embed()
            await panel_channel.send(embed=embed, view=MatchingView())
            print("✨ 募集パネルを自動設置しました！", flush=True)
